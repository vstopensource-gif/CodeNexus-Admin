...
async function loadRegistrations() {
  ...
  try {
    ...
    if (allRegistrations.length === 0) {
      tbody.innerHTML = '<tr><td colspan="5" class="no-data">No registrations found in database</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = `<tr><td colspan="5" class="error">Error loading registrations: ${error.message}</td></tr>`;
  }
}
...
function renderRegistrationsTable(registrations) {
  ...
  if (!registrations || registrations.length === 0) {
    tbody.innerHTML = '<tr><td colspan="5" class="no-data">No registrations found</td></tr>';
    return;
  }
